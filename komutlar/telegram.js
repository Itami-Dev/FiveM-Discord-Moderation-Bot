...
module.exports.run = async (client, message, args, config) => {
  ...
  .addField(':warning: Uyarı :warning:', '`telegram` adlı komutu özel mesajlarda kullanamazsın.')
 ...
 return;
    ...
    if (hesapismi.length < 1) return message.reply(' :warning: | **Lütfen hesabının ismini belirt!**');
     ...
     db.set(`telegram_${message.author.id}`, hesapismi)

      message.channel.send(`${message.author.tag}, Başarıyla Telegram hesabının isimini güncelledim!`)
