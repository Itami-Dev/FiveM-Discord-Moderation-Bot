...
module.exports.run = async (client, message, args, config) => {
  ...
  .addField(':warning: Uyarı :warning:', '`facebook` adlı komutu özel mesajlarda kullanamazsın.')
  return message.author.sendEmbed(ozelmesajuyari); }
    ...
    if (hesapismi.length < 1) return message.reply(' :warning: | **Lütfen hesabının ismini belirt!**');
     ...
     db.set(`facebook_${message.author.id}`, hesapismi)

      message.channel.send(`${message.author.tag}, Başarıyla Facebook hesabının isimini güncelledim!`)
