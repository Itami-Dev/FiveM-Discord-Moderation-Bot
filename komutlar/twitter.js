...
module.exports.run = async (client, message, args, config) => {
  ...
  .addField(':warning: Uyarı :warning:', '`twitter` adlı komutu özel mesajlarda kullanamazsın.')
 ...
 return;
    ...
    if (hesapismi.length < 1) return message.reply(' :warning: | **Lütfen hesabının ismini belirt!**');
     ...
     db.set(`twitter_${message.author.id}`, hesapismi)

      message.channel.send(`${message.author.tag}, Başarıyla Twitter hesabının isimini güncelledim!`)
